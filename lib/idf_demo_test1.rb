...
class WordIdf < ActiveRecord::Base
  self.table_name = "word_idfs_test"
  def self.save(word, article_id)
    if !word.blank?
      wi = WordIdf.find_by_word(word)
      if wi.nil?
        wi2 = WordIdf.new({ :word => word, :article_id => article_id })
        wi2.save
      else
